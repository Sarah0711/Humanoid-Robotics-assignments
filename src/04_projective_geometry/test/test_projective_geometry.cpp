...
TEST(ProjectiveGeometry, euclideanToHomogeneous) 
{
	...
	ASSERT_DOUBLE_EQ(1, (double) result(0));
	ASSERT_DOUBLE_EQ(3, (double) result(1));
	ASSERT_DOUBLE_EQ(2, (double) result(2));
	ASSERT_DOUBLE_EQ(1, (double) result(3));
}
TEST(ProjectiveGeometry, homogeneousToEuclidean) 
{
	...
	ASSERT_DOUBLE_EQ(2, (double) result(0));
	ASSERT_DOUBLE_EQ(3, (double) result(1));
}
TEST(ProjectiveGeometry, setCameraParameters) 
{
	CameraParameters result = ProjectiveGeometry::setCameraParameters(4.0*PI/180.0);
	ASSERT_DOUBLE_EQ(400, (double) result.xH);
	ASSERT_DOUBLE_EQ(300, (double) result.yH);
	ASSERT_DOUBLE_EQ(0.0025, (double) result.m);
	ASSERT_DOUBLE_EQ(550, (double) result.c);
	ASSERT_DOUBLE_EQ(0.4, (double) result.X0(0));
	ASSERT_DOUBLE_EQ(0, (double) result.X0(1));
	ASSERT_DOUBLE_EQ(10, (double) result.X0(2));
	ASSERT_DOUBLE_EQ(0, (double) result.rotX);
	ASSERT_DOUBLE_EQ(4.0*PI/180.0, (double) result.rotY);
	ASSERT_DOUBLE_EQ(0, (double) result.rotZ);
}
TEST(ProjectiveGeometry, calibrationMatrix) 
{
	...
	ASSERT_DOUBLE_EQ(570, (double) result(0,0));
	ASSERT_DOUBLE_EQ(0, (double) result(0,1));
	ASSERT_DOUBLE_EQ(500, (double) result(0,2));
	ASSERT_DOUBLE_EQ(0, (double) result(1,0));
	ASSERT_DOUBLE_EQ(571.254, (double) result(1,1));
	ASSERT_DOUBLE_EQ(270, (double) result(1,2));
	ASSERT_DOUBLE_EQ(0, (double) result(2,0));
	ASSERT_DOUBLE_EQ(0, (double) result(2,1));
	ASSERT_DOUBLE_EQ(1, (double) result(2,2));
}
...
TEST(ProjectiveGeometry, reprojectTo3D) {
	const double epsilon = 0.001;
	const double tableHeight = 0.20;
	const double x = 3.4513;
	const double y = 2.1163;
	CameraParameters param;
	param.xH = 500;
	param.yH = 270;
	param.m = 0.0022;
	...
	Eigen::Matrix3d calibrationMatrix;
	calibrationMatrix<<570,0,500,0,571.254,270,0,0,1;
	const Eigen::Vector2d imagePoint = Eigen::Vector2d(412, 161);
	const Eigen::Vector3d reprojection = ProjectiveGeometry::reprojectTo3D(imagePoint, calibrationMatrix, param, tableHeight);
	ASSERT_NEAR(tableHeight, (double) reprojection(2), epsilon);
	ASSERT_NEAR(x, (double) reprojection(0), epsilon);
	ASSERT_NEAR(y, (double) reprojection(1), epsilon);
