...
TEST(ParticleFilter, gaussianProbability) {
	...
	ASSERT_NEAR(0.398942, ParticleFilter::gaussianProbability(0, 1), epsilon);
	ASSERT_NEAR(0.106483, ParticleFilter::gaussianProbability(-2, 3), epsilon);
}
...
TEST(ParticleFilter, sampleFromGaussian) {
	...
	for (size_t j = 0; j < 1000; ++j) {
		...
		if (stdev < 1e-5) {
			if (mean == 0.0) {
				FAIL() << "The method does not do anything.";
			} else if (fabs(samples[0] - real_mean) < 1e-5) {
				FAIL() << "The method always returns the mean.";
			} else {
				FAIL() << "sampleFromGaussian(" << real_mean << ", " << real_stdev << ") always returns the same value " << samples[0] << ". DO NOT call srand() in this function!";
			}
		}
        ...
        const double q = boost::math::quantile(boost::math::complement(student, 0.5 * alpha));
#else
		const double q = 2.62640545728;
#endif
        const double width = q * real_stdev / sqrt((double) n);
...
#else
			const double lower_limit = 66.5101053017;
			const double upper_limit = 138.986783451;
#endif
...
TEST(ParticleFilter, initParticles) {
	...
	ASSERT_DOUBLE_EQ(sumOfWeights, 1.);

	std::vector<ParticleFilter::Particle> oneParticle(1);
	ParticleFilter::initParticles(oneParticle);
	ParticleFilter::Particle first = oneParticle[0];
	bool allSame = true;
	for (size_t i = 0; allSame && i < 10; ++i) {
		ParticleFilter::initParticles(oneParticle);
		if (fabs(oneParticle[0].x - first.x) > 1e-5) {
			allSame = false;
		}
	}
	if (allSame) {
		FAIL() << "The method always creates the same set of particles instead of a random distribution. DO NOT call srand() in this function!";
	}
}
...
TEST(ParticleFilter, normalizeWeights) {
	...
	const double s = 2.0 / ((double) (particles.size() * (particles.size() + 1)));
	for (size_t i = 0; i < particles.size(); ++i) {
...
#ifdef Boost_RANDOM_FOUND
	const double alpha = 0.01;  // confidence level: 99%
#endif
	const double ux = 2.0;
...
TEST(ParticleFilter, getDistanceToNearestLight) {
	const double epsilon = 0.00001;
	const size_t n = 21;
	const double values[n] = {2, 1.5, 1, 0.5, 0, 0.5, 1, 1.5, 2, 1.5, 1, 0.5, 0, 0.5, 1, 0.5, 0, 0.5, 1, 1.5, 2};
	const double dx = 0.5;
	double x = 0.0;
	bool hasNonzero = false;
	for (size_t i = 0; i < n && !hasNonzero; ++i) {
	...
	x = 0.0;
	for (size_t i = 0; i < n; ++i) {
		ASSERT_NEAR(values[i], ParticleFilter::getDistanceToNearestLight(x), epsilon);
		x += dx;
	}
}
...
TEST(ParticleFilter, integrateObservation) {
	...
	ASSERT_NEAR(0.18877, particles[0].weight, epsilon);
	ASSERT_NEAR(0.31123, particles[1].weight, epsilon);
	ASSERT_NEAR(0.18877, particles[2].weight, epsilon);
	ASSERT_NEAR(0.31123, particles[3].weight, epsilon);
}
