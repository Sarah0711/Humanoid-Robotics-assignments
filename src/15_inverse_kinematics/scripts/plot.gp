...
stats "../data/glass.txt" nooutput
numLines=STATS_records
...
do for [i=0:numLines-1] {
    set table "../data/tmp.txt"
    plot "../data/glass.txt" every ::i::i using 1:(q0=$1):(q1=$2):(q2=$3)
    ...
    plot tx + (t < 0.5*pi ? rx1 * cos(t) : (rx2 + k*(t-0.5*pi)) * cos(2.0 * pi - t)),ty + (t < 0.5*pi ? ry1 * sin(t) : ry1 + ry2 + ry2 * sin(2.0 * pi - t)) w l lw 2 title "planned trajectory", \
       "../data/glass.txt" every ::0::i using 4:5 with line lw 2 lc 3 title "executed trajectory"
    pause 0.01
