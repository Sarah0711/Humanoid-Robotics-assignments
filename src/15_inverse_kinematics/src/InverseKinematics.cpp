...
EndeffectorPose InverseKinematics_2Links::forwardKinematic(const JointAngles& q) const {
	...
	Eigen::Vector3d grip = Eigen::Vector3d::Zero(3);
	grip<<h,0,1;
	Eigen::Vector3d e_homo = rotation(q(0))*translation(a0,0)*rotation(q(1))*translation(a1,0)*grip;
...
Jacobian InverseKinematics_2Links::jacobian(const JointAngles& q) const {
	...
	result << -((h+a1)*sin(q(0) + q(1)) + a0*sin(q(0))), -(h+a1)*sin(q(0) + q(1)), (h+a1)*cos(q(0) + q(1)) + a0*cos(q(0)), (h+a1)*cos(q(0) + q(1));
...
void InverseKinematics::oneIteration(JointAngles& q, EndeffectorPose& e, const EndeffectorPose& g) const {
	 ...
	 * endeffector pose e.
	 */
	q =applyChangeToJoints(q , computeJointChange(chooseStep(e,g), jacobian(q)));
...
JointAngles InverseKinematics_2Links::computeIK(const EndeffectorPose& g, const double& maxTranslationalError) const {
	...
	while( sqrt((e(0)-g(0))*(e(0)-g(0)) + (e(1)-g(1))*(e(1)-g(1))) > maxTranslationalError){
		oneIteration(q,e,g);
	}
	return q;
}
...
EndeffectorPose InverseKinematics_3Links::forwardKinematic(const JointAngles& q) const {
	...
	Eigen::Vector3d e_homo = rotation(q(0))*translation(a0,0)*rotation(q(1))*translation(a1,0)*rotation(q(2))*translation(a2,0)*grip;

	e<<e_homo(0),e_homo(1),q(0)+q(1)+q(2);
	return e;
}
...
Jacobian InverseKinematics_3Links::jacobian(const JointAngles& q) const {
	 ...
	 */
	JointAngles q0_eps = Eigen::VectorXd::Zero(3);
	q0_eps<<q(0) + epsilon,q(1),q(2);

	JointAngles q1_eps = Eigen::VectorXd::Zero(3);
	q1_eps<<q(0),q(1) + epsilon,q(2);

	JointAngles q2_eps = Eigen::VectorXd::Zero(3);
	q2_eps<<q(0),q(1),q(2) + epsilon;



	result << (forwardKinematic(q0_eps) - forwardKinematic(q))/epsilon, (forwardKinematic(q1_eps) - forwardKinematic(q))/epsilon, (forwardKinematic(q2_eps) - forwardKinematic(q))/epsilon;
...
JointAngles InverseKinematics_3Links::computeIK(const EndeffectorPose& g, const double& maxTranslationalError, const double& maxAngularError) const {
	...
	EndeffectorPose e = forwardKinematic(q);
	while( sqrt((e(0)-g(0))*(e(0)-g(0)) + (e(1)-g(1))*(e(1)-g(1))) > maxTranslationalError || (e(2)-g(2) > maxAngularError) ){
		oneIteration(q,e,g);
	}
