...
void FileIO::logFailedExtend(const GridNode * const from, const GridNode * const randomNode, const std::string * const listName) {
	if (logfile.good()) {
		logfile << ((GridNode *) from)->toLogString() << " " << std::numeric_limits<double>::quiet_NaN() << " " << std::numeric_limits<double>::quiet_NaN() << " " << ((GridNode *) randomNode)->toLogString() << " " << (*listName) << std::endl;
	}
}
