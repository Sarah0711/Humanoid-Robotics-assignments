...
public:
	IRMTest() {
		...
		for (size_t i = 0; i < 2; ++i) {
			for (size_t j = 0; j < 2; ++j) {
				voxels[i].configurations.push_back(new JointConfiguration(testcases[2*i+j].jointAngles, testcases[2*i+j].manipulability));
			}
		}
		numAdded = 0;
	}
	~IRMTest() {
		for (size_t i = 0; i < 2; ++i) {
			for (size_t j = 0; j < 2; ++j) {
				delete voxels[i].configurations[j];
			}
			voxels[i].configurations.clear();
		}
	}
...
/*
TEST(Internal, bestManipulability) {
	AbstractIRM::RMEntry entry(Eigen::Vector3d::Zero());
	entry.addConfiguration(Eigen::Vector3d(1., 1., 1.), 0.2, Eigen::Vector3d(10., 10., 10.));
	entry.addConfiguration(Eigen::Vector3d(2., 2., 2.), 0.6, Eigen::Vector3d(20., 20., 20.));
	entry.addConfiguration(Eigen::Vector3d(3., 3., 3.), 0.4, Eigen::Vector3d(30., 30., 30.));
	ASSERT_TRUE(entry.getBestConfiguration() != NULL);
	EXPECT_DOUBLE_EQ(entry.getBestConfiguration()->manipulability, 0.6);
	EXPECT_DOUBLE_EQ(static_cast<double>(entry.getBestConfiguration()->jointAngles[0]), 2.);
	EXPECT_DOUBLE_EQ(static_cast<double>(entry.getEndeffectorPose()[0]), 20.);
}
*/
